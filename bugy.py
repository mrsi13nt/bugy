...
def one(domain,notify):
    ...
    if notify == "y":
        typingPrint("that's nice :)")
        clear()
        typingPrint("now we using subfinder\n")
        subprocess.run("echo 'now we using subfinder' | notify -silent",shell=True)
        print(subprocess.run("subfinder -d "+ domain +" -o outputs/finder_subs.txt | notify -bulk -silent -data outputs/finder_subs.txt",shell=True))
        clear()
        typingPrint("now we using amass\n")
        subprocess.run("echo 'now we using amass' | notify -silent",shell=True)
        print(subprocess.run("amass enum -passive -d "+domain+" -o outputs/amass_subs.txt | notify -bulk -silent -data outputs/amass_subs.txt",shell=True))
        clear()
        typingPrint("now we using assetfinder\n")
        subprocess.run("echo 'now we using assetfinder' | notify -silent",shell=True)
        print(subprocess.run("assetfinder -subs-only "+domain+" > outputs/asset_subs.txt | notify -bulk -silent -data outputs/asset_subs.txt",shell=True))
        clear()
        ...
        print(subprocess.run("cat outputs/finder_subs.txt outputs/amass_subs.txt outputs/asset_subs.txt | sort -u >> outputs/uniq_subs.txt",shell=True))
        print(subprocess.run("cat outputs/uniq_subs.txt | httpx -o outputs/httpx.txt",shell=True))
        print(subprocess.run("cat outputs/uniq_subs.txt | httprobe >> outputs/subdomains.txt",shell=True))
        subprocess.run("echo 'results of httpx' | notify -bulk -silent -data outputs/httpx.txt",shell=True)
        clear()
        ...
        subprocess.run("echo 'now we using dirb' | notify -silent",shell=True)
        with open(file_path_probe,'r') as file:
            for line in file:
                line = line.replace("\n", "")
                print(subprocess.run("dirb "+line+" -f -o outputs/dirb.txt | notify -bulk -silent -data outputs/dirb.txt",shell=True), end='')
        clear()
        ...
        with open(file_path_probe,'r') as file:
            for line in file:
                line = line.replace("\n", "")
                print(subprocess.run("dirsearch -e php,html,js,jsp,bak,zip,tgz,txt -u "+line+" -t 10 -o outputs/dirsearch.txt | notify -bulk -silent -data outputs/dirsearch.txt",shell=True), end='')
        clear()
        ...
        with open(file_path_probe,'r') as file:
            for line in file:
                line = line.replace("\n", "")
                print(subprocess.run("gobuster dir -u "+line+" --no-error -r -o outputs/gobuster.txt | notify -bulk -silent -data outputs/gobuster.txt",shell=True), end='')
        clear()
        ...
        with open(file_path_probe,'r') as file:
            for line in file:
                line = line.replace("\n", "")
                print(subprocess.run("ffuf -u "+line+"/FUZZ -w /usr/share/Seclists/Discovery/Web-content/raft-medium-files.txt -mc 200,302,301 -t 1000 -o outputs/ffuf.txt | notify -bulk -silent -data outputs/ffuf.txt"), end='')
        clear()
        ...
        print(subprocess.run("arjun -i outputs/dir.txt -oT outputs/arjun.txt | notify -bulk -silent -data outputs/arjun.txt",shell=True))
        clear()
        typingPrint("now we using gospider")
        subprocess.run("echo 'now we using gospider' | notify -silent",shell=True)
        print(subprocess.run("gospider -S outputs/dir.txt -a -w -o outputs/gospider.txt | notify -bulk -silent -data outputs/gospider.txt",shell=True))
        clear()
        ...
        print(subprocess.run("cat outputs/arjun.txt outputs/gospider.txt >> outputs/part3_output.txt",shell=True))
        clear()
        typingPrint("now we using waybackuls")
        subprocess.run("echo 'now we using waybackurls' | notify -silent",shell=True)
        print(subprocess.run("cat outputs/dir.txt | waybackurls >> outputs/waybackurls.txt | notify -bulk -silent -data outputs/waybackurls.txt",shell=True))
        clear()
        typingPrint("now we using gau")
        subprocess.run("echo 'now we using gau' | notify -silent",shell=True)
        print(subprocess.run("cat outputs/dir.txt | gau >> outputs/gau.txt | notify -bulk -silent -data outputs/gau.txt",shell=True))
        clear()
        typingPrint("now we using katana")
        subprocess.run("echo 'now we using katana' | notify -silent",shell=True)
        print(subprocess.run("katana -list outputs/dir.txt -v -jc -o outputs/katana.txt | notify -bulk -silent -data outputs/katana.txt",shell=True))
        clear()
        typingPrint("now we using hakrawler")
        subprocess.run("echo 'now we using hakrawler' | notify -silent",shell=True)
        print(subprocess.run("cat outputs/dir.txt | hakrawler >> outputs/hakrawler.txt | notify -bulk -silent -data outputs/hakrawler.txt",shell=True))
        clear()
        ...
        clear()
        typingPrint("now we using amass\n")
        print(subprocess.run("amass enum -passive -d "+domain+" -o outputs/amss_subs.txt",shell=True))
        clear()
        typingPrint("now we using assetfinder\n")
        ...
        with open(file_path_probe,'r') as file:
            for line in file:
                line = line.replace("\n", "")
                print(subprocess.run("dirb "+line+" -f -o outputs/dirb.txt",shell=True), end='')
        clear()
        typingPrint("now we using dirsearch")
        with open(file_path_probe,'r') as file:
            for line in file:
        ...
        with open(file_path_probe,'r') as file:
            for line in file:
                line = line.replace("\n", "")
                print(subprocess.run("ffuf -u "+line+"/FUZZ -w /usr/share/Seclists/Discovery/Web-content/raft-medium-files.txt -mc 200,302,301 -t 1000"), end='')
        clear()
        typingPrint("now we finished part 2, let's keep working")
        typingPrint("starting part 3 'Parameter fuzzing and gathering'")
        time.sleep(2)
        print(subprocess.run("cat outputs/dirsearch.txt outputs/gobuster.txt outputs/dirb >> outputs/dir.txt",shell=True))
        clear()
        typingPrint("now we using arjun")
        print(subprocess.run("arjun -i outputs/dir.txt -oT outputs/arjun.txt",shell=True))
        clear()
        typingPrint("now we using gospider")
        print(subprocess.run("gospider -S outputs/dir.txt -a -w -o outputs/gospider.txt",shell=True))
        clear()
        typingPrint("we finished part 3 :)")
        typingPrint("i am glad you still here")
        typingPrint("let's keep going and finish last part")
        ...
        clear()
        typingPrint("now we using waybackuls")
        print(subprocess.run("cat outputs/dir.txt | waybackurls >> outputs/waybackurls.txt",shell=True))
        clear()
        typingPrint("now we using gau")
        print(subprocess.run("cat outputs/dir.txt | gau >> outputs/gau.txt",shell=True))
        clear()
        typingPrint("now we using katana")
        print(subprocess.run("katana -list outputs/dir.txt -v -jc -o outputs/katana",shell=True))
        clear()
        typingPrint("now we using hakrawler")
        print(subprocess.run("cat outputs/dir.txt | hakrawler >> outputs/hakrawler",shell=True))
        clear()
        typingPrint("want to read total report?  (yes/no)")
        ans = input(">> ")
        if ans == "yes" or ans == "y" or ans == "Y" or ans == "Yes":
